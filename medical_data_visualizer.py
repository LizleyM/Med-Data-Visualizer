...
def draw_cat_plot():
  ...
  # groupby probably not needed, just use col argument in .catplot()
...
def draw_heat_map():
  ...
  (df['weight'] <= df['weight'].quantile(0.975))]
    ...
    fig = df_heat.plot(figsize=(12,12)).figure
  
    # Draw the heatmap with 'sns.heatmap()'
    sns.heatmap(corr, mask=mask, vmin=-0.1, vmax=.7, square=True, annot=True, linewidths=1, fmt='.1f')
